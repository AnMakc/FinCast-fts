...
class FFMConfig:
  """Config for initializing ffm patched_decoder class."""
...
class TimesFMAttention(nn.Module):
  """We use the attention used in TimesFM."""
...
class TimesFMDecoderLayer(nn.Module):
  ...
  #extra todo, make ffm parameters and put this into it.
  ...
  def __init__(
    ...
    self.moe = SparseMoEBlock(moe, add_ff_before = False, add_ff_after = False) #sparse MOE block
  ...
  def forward(
    ...
    #change this to MOE architecture from MLP to MOE

    hidden_states, total_aux_loss, balance_loss, router_z_loss = self.moe(hidden_states, paddings=paddings) #update padding mask positions

    #old padding methods, outdated
...
class StackedDecoder(nn.Module):
  ...
  def forward(
    ...
    for i in range(len(self.layers)):
      ...
      _, hidden_states, cur_total_aux_loss = layer(
          hidden_states=hidden_states,
          mask=mask,
          paddings=paddings,
          kv_write_indices=kv_write_indices,
      ...
      if self.training:
        total_aux_loss += cur_total_aux_loss
...
class PatchedTimeSeriesDecoder_MOE(nn.Module):
  ...
  def __init__(self, config: FFMConfig):
    ...
    self.stacked_transformer = StackedDecoder(
        ...
        gating_top_n=self.config.gating_top_n,
        threshold_train=self.config.threshold_train,
        threshold_eval=self.config.threshold_eval
    )
    if self.config.use_positional_embedding:
      self.position_emb = PositionalEmbedding(self.config.hidden_size)

  def _forward_transform( #std normalization
  ...
  def _preprocess_input(
    ...
    patched_inputs = torch.where( #check every single element then apply input masking.
  ...
  def forward(
    ...
    return output_ts, total_aux_loss #total aux loss, can choose to ignore it when training.
  ...
  def decode(
    ...
    for step_index in range(num_decode_patches):
      ...
      fprop_outputs, _aux_loss = self(input_ts, input_padding, freq) #aux loss modification
